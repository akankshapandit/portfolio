       ...
       className='grid [grid-template-columns:repeat(auto-fit,_minmax(200px,_1fr))] gap-6 my-10'>
        {serviceData.map(({icon,title,description,link},index)=>(
            ...
            whileHover={{scale:1.05}}
             key={index} className='border border-gray-400 rounded-lg px-8 py-12 shadow hover:shadow-black cursor-pointer hover:bg-gray-100 hover:-translate-y-1 duration-500'>
                <Image src={icon} alt='' className='w-10'/>
                <h3 className='text-lg my-4 text-gray-700'>{title}</h3>
                <p className='text-sm text-gary-600 leading-5'>{description}</p>
